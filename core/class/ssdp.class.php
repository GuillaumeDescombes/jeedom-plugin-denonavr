 ...
 * Updated by Guillaume Descombes 2022
 *  
 ...
 */
 ...
 class phpSSDP {	
   ...
   * @param string  $ip     IP of the target 
	 * 
	 * @return	array|null			Array of devices or null if no devices are detected
	 */
	private static function _search($st, $timeout, $mx = null, $ip = '') {
		if(is_null($mx)) $mx = $timeout;
    if ($ip=='') $ip = '239.255.255.250';
    ...
    //bind to 1900 local port
    socket_bind ($socket, 0, 1900);
    //echo "send packet to ip '$ip'\n";
    $send_ret = socket_sendto($socket, $headers, 1024, 0, $ip, 1900);
		while(@socket_recvfrom($socket, $response, 1024, MSG_WAITALL, $_tmpIP, $_tmpPort)) {
      //echo "Received one packet from '$_tmpIP':'$_tmpPort' ....\n";
			$ret=[];
	 ...
	 */
	public static function getAllDevices($json = false, $timeout = 10, $mx = null) {
	...
	/**
	 * Callable static method to fetch all services of a device identified by its IP address.
	 *    
	 * @param	boolean		$json		Triggers a JSON output
	 * @param	int			$timeout	Defines the timeout value in seconds
	 * @param	int			$mx			MX value in seconds (Response delay for devices)
   * @param boolean $returnOnlyDescription  return only the description of the device
	 * 
	 * @return	array		An array the devices list and full description
	 */
 
  public static function getDeviceByIP($ip, $json = false, $timeout = 1, $mx = null, $returnOnlyDescription=true) {
    $devices = phpSSDP::_search('ssdp:all', $timeout, $mx, $ip);
    $deviceFound = array();
    $deviceInfo = null;
    if (is_array($devices)) {
      foreach($devices as $key => $device) {
        if ($device['IP'] == $ip) {
          if ($deviceInfo == null) $deviceInfo = self::_getDeviceInfo($device['LOCATION']);		
          $device['DESCRIPTION'] = $deviceInfo;
          $deviceFound[]=$device;
          if ($deviceInfo != null && $returnOnlyDescription) {
            return self::_sendResponse($deviceInfo, $json);
          }
        }
      }
    }
    return self::_sendResponse($deviceFound, $json);
  }
}
